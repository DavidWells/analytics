...
async function getPluginInfo(filePath, otherParam) {
  const code = await readFile(filePath, 'utf-8')
  let jsDocData = []
  let ast = { methodsByName: [], methodsAndValues: [], foundExports: [] }
  try {
    // console.log(`work filePath`, filePath)
    // console.log('otherParam', otherParam)
    jsDocData = await getJSDocInfo(filePath)
    ast = parseCode(code)
  } catch (err) {
    console.log(`fail filePath`, filePath)
    // console.log('otherParam', otherParam)
    console.log('err', err)
  }
  return {
