...
const CONSTANTS = {
  FIRST_SOURCE: '__user_first_referrer'
}

const EVENTS = {
  REFERRAL_SRC: 'firstReferralSource'
}
...
function firstReferralSource(referrer) {
  ...
  const cookie = getCookie(CONSTANTS.FIRST_SOURCE)
  if (cookie) {
  ...
  if (utmzCookie) {
    ...
    if (parsedCookie) {
      setCookie(CONSTANTS.FIRST_SOURCE, formatCookie(parsedCookie))
      return parsedCookie
    }
  }
  ...
  if (refData) {
    setCookie(CONSTANTS.FIRST_SOURCE, formatCookie(refData))
  }
  return refData
}
