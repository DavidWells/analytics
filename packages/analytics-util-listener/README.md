...
[Tiny](https://bundlephobia.com/result?p=@analytics/listener-utils) form utilities library for dealing with HTML form data.
...
This library is backwards compatible back to IE 8.

## Why this package?

This package makes it a little easy to work with `addEventListener` & `removeEventListener` by returning a clean up function for both. This makes it easy to reAttach a listener or disable a listener with it's return function.

Additionally this package is backwards compatible with older browsers.
...
Below is the api for `@analytics/listener-utils`. You can import only what you need & the rest will be tree-shaken out of your bundle.
