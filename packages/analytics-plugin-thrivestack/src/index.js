function thriveStackPlugin(pluginConfig = {}) {
            ...
            if (step.stepId === 'name_your_website' && step.value) {
              try {
                ...
                if (stepValue.website_urls && Array.isArray(stepValue.website_urls)) {
                  ...
                  const isValid = normalizedUrls.some(domain => 
                    currentHost.includes(domain) || domain.includes(currentHost)
                  );
  ...
  const plugin = {
    ...
    initialize: async ({ config, instance: analyticsInstance }) => {
    ...
    identify: ({ payload }) => {
      if (!initCompleted || !window.ThriveStack) {
        window._tsq.push(["identify", { payload }]);
        return;
      }
      ...
      try {
        ...
        window.ThriveStack.identify(identifyPayload)
          .then(result => console.log('Identify sent successfully:', result))
          .catch(err => {
    ...
    track: ({ payload }) => {
      if (!initCompleted || !window.ThriveStack) {
        window._tsq.push(["track", { payload }]);
        return;
      }
      ...
      try {
        ...
        const eventPayload = [{
          ...
          context: {
            group_id: groupId,
            device_id: deviceId || '', // Use empty string only when sending the payload
    ...
    page: ({ payload }) => {
      if (!initCompleted || !window.ThriveStack) {
        window._tsq.push(["page", { payload }]);
        return;
      }

      const { properties = {}, options = {} } = payload;
    ...
    methods: {
      ...
      group: (groupId, traits = {}, options = {}, callback) => {
        ...
        return window.ThriveStack.group(groupPayload)
          .then(result => {
            if (callback && typeof callback === 'function') {
